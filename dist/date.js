...
function getWeekdays(locale) {
    ...
    return Object.freeze(Array.from({ length: 7 }, (_, i) => fmt.format(new Date(1970, 0, 4 + i))));
}
function getWeekdaysShort(locale) {
    const fmt = getFormatter(WEEKDAY_SHORT, locale, { weekday: "short" });
    return Object.freeze(Array.from({ length: 7 }, (_, i) => fmt.format(new Date(1970, 0, 4 + i))));
}
function getWeekday(input, locale) {
    return getFormatter(WEEKDAY_LONG, locale, { weekday: "long" }).format(resolve(input));
}
function getWeekdayShort(input, locale) {
    return getFormatter(WEEKDAY_SHORT, locale, { weekday: "short" }).format(resolve(input));
}
function getMonths(locale) {
    const fmt = getFormatter(MONTH_LONG, locale, { month: "long" });
    return Object.freeze(Array.from({ length: 12 }, (_, i) => fmt.format(new Date(1970, i, 1))));
}
function getMonthsShort(locale) {
    const fmt = getFormatter(MONTH_SHORT, locale, { month: "short" });
    return Object.freeze(Array.from({ length: 12 }, (_, i) => fmt.format(new Date(1970, i, 1))));
}
function getMonth(input, locale) {
    return getFormatter(MONTH_LONG, locale, { month: "long" }).format(resolve(input));
}
function getMonthShort(input, locale) {
    return getFormatter(MONTH_SHORT, locale, { month: "short" }).format(resolve(input));
}
/* ----------------------- Date Manipulation ----------------------- */
function addSeconds(seconds = 0, input, asString) {
    const d = resolve(input);
    if (validate_1.default.isNumber(seconds))
        d.setSeconds(d.getSeconds() + seconds);
    return asString ? d.toLocaleString() : d;
}
function addMinutes(minutes = 0, input, asString) {
    const d = resolve(input);
    if (validate_1.default.isNumber(minutes))
        d.setMinutes(d.getMinutes() + minutes);
    return asString ? d.toLocaleString() : d;
}
function addHours(hours = 0, input, asString) {
    const d = resolve(input);
    if (validate_1.default.isNumber(hours))
        d.setHours(d.getHours() + hours);
    return asString ? d.toLocaleString() : d;
}
function addDays(days = 0, input, asString) {
    const d = resolve(input);
    if (validate_1.default.isNumber(days))
        d.setDate(d.getDate() + days);
    return asString ? d.toLocaleString() : d;
}
function addMonths(months = 0, input, asString) {
    const d = resolve(input);
    if (validate_1.default.isNumber(months))
        d.setMonth(d.getMonth() + months);
    return asString ? d.toLocaleString() : d;
}
function addYears(years = 0, input, asString) {
    const d = resolve(input);
    if (validate_1.default.isNumber(years))
        d.setFullYear(d.getFullYear() + years);
    return asString ? d.toLocaleString() : d;
}
function addWorkDays(workdays = 0, input, asString) {
    const d = resolve(input);
    if (validate_1.default.isNumber(workdays)) {
...
function getMostRecentWorkDate(input, asString) {
    const d = resolve(input);
    const day = d.getDay();
...
function getRelative(input, asShort, locale) {
    ...
    const diffMonth = (d.getFullYear() - now.getFullYear()) * 12 +
    ...
    if (Math.abs(diffSec) < 15) {
        ...
        return asShort ? `${diffYear}y` : rtf.format(diffYear, "year");
}
/* --------------------------- Validators -------------------------- */
function isDay(input, option = {}) {
    const { date, addYears = 0, addMonths = 0, addDays = 0 } = option;
    const dateA = resolve(input);
    const dateB = resolve(date);
    return (dateA.getFullYear() === dateB.getFullYear() + addYears &&
        dateA.getMonth() === dateB.getMonth() + addMonths &&
        dateA.getDate() === dateB.getDate() + addDays);
}
function isPast(input1, input2) {
    return resolve(input1) < resolve(input2);
}
function isPresent(input1, input2) {
    return resolve(input1) === resolve(input2);
}
function isFuture(input1, input2) {
    return resolve(input1) > resolve(input2);
}
function isDayPast(input1, input2) {
    const d1 = resolve(input1);
    const d2 = resolve(input2);
    return (isMonthPast(d1, d2) ||
        (isMonthPresent(d1, d2) && d1.getDate() < d2.getDate()));
}
function isDayPresent(input1, input2) {
    const d1 = resolve(input1);
    const d2 = resolve(input2);
    return isMonthPresent(d1, d2) && d1.getDate() === d2.getDate();
}
function isDayFuture(input1, input2) {
    const d1 = resolve(input1);
    const d2 = resolve(input2);
    return (isMonthFuture(d1, d2) ||
        (isMonthPresent(d1, d2) && d1.getDate() > d2.getDate()));
}
function isMonthPast(input1, input2) {
    const d1 = resolve(input1);
    const d2 = resolve(input2);
    return (isYearPast(d1, d2) ||
        (isYearPresent(d1, d2) && d1.getMonth() < d2.getMonth()));
}
function isMonthPresent(input1, input2) {
    const d1 = resolve(input1);
    const d2 = resolve(input2);
    return isYearPresent(d1, d2) && d1.getMonth() === d2.getMonth();
}
function isMonthFuture(input1, input2) {
    const d1 = resolve(input1);
    const d2 = resolve(input2);
    return (isYearFuture(d1, d2) ||
        (isYearPresent(d1, d2) && d1.getMonth() > d2.getMonth()));
}
function isYearPast(input1, input2) {
    return resolve(input1).getFullYear() < resolve(input2).getFullYear();
}
function isYearPresent(input1, input2) {
    return resolve(input1).getFullYear() === resolve(input2).getFullYear();
}
function isYearFuture(input1, input2) {
    return resolve(input1).getFullYear() > resolve(input2).getFullYear();
}
...
function isWeekdayName(input, locale) {
...
function isMonthName(input, locale) {
    if (validate_1.default.isStringNonEmpty(input)) {
        const v = input.toLowerCase();
...
function resolve(input) {
    return validate_1.default.isDate(input) ? new Date(input) : new Date();
}
function getFormatter(cache, locale = "en-US", options) {
    var _a;
    return ((_a = cache[locale]) !== null && _a !== void 0 ? _a : (cache[locale] = new Intl.DateTimeFormat(locale, options)));
}
/* ----------------------------- Export ---------------------------- */
exports.default = {
    // Constants / Caches
    ...
    validate: validate_1.default.isDate, // simple date validation
    // Internals / Utilities
